      ...
      {lightbox && (
        <div className="fixed inset-0 z-[100] bg-ink/95 backdrop-blur-md flex flex-col animate-fade-in" onClick={() => setLightbox(false)}>
          <div className="flex items-center justify-between px-5 md:px-8 py-4 text-bone/70 text-[10px] md:text-xs uppercase tracking-[0.3em] font-mono">
            ...
            <span>{String(active + 1).padStart(2, "0")} <span className="text-bone/40 mx-1">—</span> {String(slides.length).padStart(2, "0")}</span>
          ...
          <div className="relative flex-1 flex items-center justify-center px-4 md:px-20 pb-8" onClick={(e) => e.stopPropagation()}>
            {slides[active].type === 'video' ? (
              (() => {
                ...
                if (!v) return null;
                return v.kind === "iframe" ? (
                  <iframe src={v.src} title="Vidéo du bien" allowFullScreen className="w-full h-full" />
                ) : (
                  <video src={v.src} controls className="max-w-full max-h-full object-contain" />
                );
              })()
            ) : (
              <img key={active} src={slides[active].src} alt={property.name} className="max-w-full max-h-full object-contain animate-fade-in" />
            )}
            {slides.length > 1 && (
              <>
                <button
